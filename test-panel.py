...
class MainWindow:
    ...
    def create_widgets(self):
        ...
        self.control_panel = tk.Frame(self.main_frame, bg="lightgray", width=400)
#        self.control_panel.grid_propagate(False)
        ...
        self.g_view_text = tk.Text(self.control_panel, wrap=tk.WORD, height=11, width=5)  # widthを指定しないとパネルの幅が広がってしまう
        self.g_view_text.grid(row=0, column=0, padx=3, pady=(3,0), sticky="ew")
        self.g_view_text.insert("1.0", "★仮表示★\n本来は、\nキャンバスの\nミニマップを表示")
        ...
        # 漸化式入力
        zanka_label = tk.Label(self.plugin_frame1, text="漸化式:", bg="lightblue")
        zanka_label.grid(row=0, column=0, padx=3, pady=3, sticky="ew")
        self.zanka_entry = ttk.Entry(self.plugin_frame1)
        self.zanka_entry.grid(row=0, column=1, padx=3, pady=3, sticky="ew")
        # 反復回数入力
        hanpuku_label = tk.Label(self.plugin_frame1, text="反復回数:", bg="lightblue")
        hanpuku_label.grid(row=1, column=0, padx=3, pady=3, sticky="ew")
        self.hanpuku_entry = ttk.Entry(self.plugin_frame1)
        self.hanpuku_entry.grid(row=1, column=1, padx=3, pady=3, sticky="ew")
        self.plugin_frame1.columnconfigure(1, weight=1)
        ...
        # Z-実・Z-虚入力
        z_real_label = tk.Label(self.plugin_frame2, text="Z-実:", bg="lightblue")
        z_real_label.grid(row=0, column=0, padx=3, pady=3, sticky="w")
        self.z_real_entry = ttk.Entry(self.plugin_frame2)
        self.z_real_entry.grid(row=0, column=1, padx=3, pady=3, sticky="ew")
        z_imag_label = tk.Label(self.plugin_frame2, text="Z-虚:", bg="lightblue")
        z_imag_label.grid(row=0, column=2, padx=3, pady=3, sticky="w")
        self.z_imag_entry = ttk.Entry(self.plugin_frame2)
        self.z_imag_entry.grid(row=0, column=3, padx=3, pady=3, sticky="ew")
        # C-実・C-虚入力
        c_real_label = tk.Label(self.plugin_frame2, text="C-実:", bg="lightblue")
        c_real_label.grid(row=1, column=0, padx=3, pady=3, sticky="w")
        self.c_real_entry = ttk.Entry(self.plugin_frame2)
        self.c_real_entry.grid(row=1, column=1, padx=3, pady=3, sticky="ew")
        c_imag_label = tk.Label(self.plugin_frame2, text="C-虚:", bg="lightblue")
        c_imag_label.grid(row=1, column=2, padx=3, pady=3, sticky="w")
        self.c_imag_entry = ttk.Entry(self.plugin_frame2)
        self.c_imag_entry.grid(row=1, column=3, padx=3, pady=3, sticky="ew")
        self.plugin_frame2.columnconfigure(1, weight=1)
        self.plugin_frame2.columnconfigure(3, weight=1)
        ...
        hassan_algo_label = tk.Label(self.color_hassan_frame, text="発散:", bg="yellow")
        hassan_algo_label.grid(row=0, column=0, padx=3, pady=3, sticky="ew")
        ...
        self.hassan_algo_combobox = ttk.Combobox(self.color_hassan_frame,
        ...
        self.hassan_grad_radio.grid(row=1, column=0, padx=3, pady=3, sticky="ew")
        ...
        self.hassan_grad_combobox.grid(row=1, column=1, padx=3, pady=3, sticky="ew")
        self.color_hassan_frame.columnconfigure(1, weight=1)
        ...
        non_hassan_algo_label = tk.Label(self.color_non_hassan_frame, text="非発散:", bg="yellow")
        non_hassan_algo_label.grid(row=0, column=0, padx=3, pady=3, sticky="ew")
        ...
        self.non_hassan_algo_combobox = ttk.Combobox(self.color_non_hassan_frame,
        ...
        self.non_hassan_grad_radio = tk.Radiobutton(self.color_non_hassan_frame,
                                                    ...
                                                    value=1, bg="lightgreen")
        self.non_hassan_grad_radio.grid(row=1, column=0, padx=3, pady=3, sticky="ew")
        ...
        self.non_hassan_grad_combobox.grid(row=1, column=1, padx=3, pady=3, sticky="ew")
        self.color_non_hassan_frame.columnconfigure(1, weight=1)
        ...
        content_label.grid(row=0, column=0, sticky="nsew", padx=3, pady=3)
        # メッセージ表示ラベル
        self.message_label = tk.Label(self.content_area, text="ここにメッセージを表示します",
                                       bd=1, relief=tk.SUNKEN, anchor=tk.W, bg="lightblue")
        self.message_label.grid(row=2, column=0, sticky="ew", padx=3, pady=(0,3))
        # グラデーション編集領域
        self.gradient_frame = tk.Frame(self.content_area, bd=1, relief=tk.SUNKEN, bg="lightgray")
        self.gradient_frame.grid(row=1, column=0, sticky="ew", padx=3, pady=(0,3))
        left_frame = tk.Frame(self.gradient_frame, bg="lightblue")
        left_frame.grid(row=0, column=0, sticky="nsew", padx=3, pady=3)
        self.grad_txt1 = tk.Label(left_frame, text="グラデーション編集（仮）メッセージ1", anchor=tk.W, bg="lightgreen")
        self.grad_txt1.grid(row=0, column=0, sticky="ew")
        self.grad_txt2 = tk.Label(left_frame, text="グラデーション編集（仮）メッセージ2", anchor=tk.W, bg="lightgreen")
        self.grad_txt2.grid(row=1, column=0, sticky="ew")
        self.grad_txt3 = tk.Label(left_frame, text="グラデーション編集（仮）メッセージ3", anchor=tk.W, bg="lightgreen")
        self.grad_txt3.grid(row=2, column=0, sticky="ew")
        # グラデーションセーブボタン
        right_frame = tk.Frame(self.gradient_frame, bg="lightblue")
        right_frame.grid(row=0, column=1, sticky="ns", padx=3, pady=3)
        self.save_button = tk.Button(right_frame, text="グラデーションセーブ", height=3)
        self.save_button.grid(row=0, column=0, padx=3, pady=3, sticky="news")
